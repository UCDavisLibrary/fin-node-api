...
`);
  ...
  async crawl(options, contentTypes) {
    ...
    if( metadata['@type'].indexOf('http://fedora.info/definitions/v4/repository#Binary') > -1 ) {
      ...
      if( options.ignoreBinary === true || this._ignoreSubPath(options) === true ) return;
    ...
    if( options.ignoreMetadata !== true && this._ignoreSubPath(options) !== true ) {
      if( binaryFile ) {
  ...
  _ignoreSubPath(options) {
    if( !options.includeFilter && !options.subPaths ) return false;
