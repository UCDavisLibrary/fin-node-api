...
`);
  ...
  async crawl(options, contentTypes) {
    ...
    parts.forEach(url => {
       ...
       */
      let containerPath = relCurrentPath;
      if( urlPath === containerPath || 
        (containerPath === '/' && urlPath === '/'+options.collectionName) // snowflake
        ) {
        ttl = ttl.replace(url, '<>');
        return;
      }

      if( containerPath !== '/' && urlPath.startsWith(containerPath) ) {
