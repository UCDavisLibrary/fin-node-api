...
class ImportCollection {
  ...
  async run(options) {
    ...
    await this.postContainers(newCollectionName, rootDir);
    ...
    if( fs.existsSync(rootMetadata) ) {
      ...
      console.log(this.getMetadata(rootMetadata));
      response = await api.put({
        path : p,
        content : this.getMetadata(rootMetadata),
        partial : true,
        headers : {
    ...
    await this.putContainers(newCollectionName, rootDir);
   ...
   * @param {IoDir} dir 
   */
  async postContainers(collectionName, dir) {
    let files = await dir.getFiles();
   ...
   * @param {IoDir} dir 
   */
  async putContainers(collectionName, dir) {
    let files = await dir.getFiles();
    ...
    for( let container of files.containers ) {
      ...
      let response = await api.put({
        ...
        content : this.getMetadata(container.localpath),
        partial : true,
        headers : {
          'content-type' : api.RDF_FORMATS.TURTLE
        }
    ...
    for( let binary of files.binaries ) {
      ...
      let response = await api.put({
        ...
        content : this.getMetadata(binary.metadata, binary.id),
        partial : true,
        headers : {
          'content-type' : api.RDF_FORMATS.TURTLE
        }
  ...
  getMetadata(path, binaryId) {
    let content = fs.readFileSync(path, 'utf-8');
    if( binaryId ) {
      content = content.replace(/<\s*>/g, '<../'+binaryId+'>');
    }
    return content;
  }
