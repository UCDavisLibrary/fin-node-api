...
class Service {
  ...
  constructor(api) {
    ...
    this.TYPES = {PROXY, FRAME, WEBHOOK, EXTERNAL, AUTHENTICATION}
    this.ROOT = ROOT;
   ...
   * @param {String} options.url url template (AuthenticationService).  ex: http://cas:8000
  ...
  async create(options) {
    ...
    if( options.type === this.TYPES.PROXY || options.type === this.TYPES.EXTERNAL ) {
      templateOptions.payload = `  fin:urlTemplate "${options.urlTemplate}" ;`
    } else if( options.type === this.TYPES.AUTHENTICATION ) {
      templateOptions.payload = `  fin:url "${options.url}" ;`
