...
class ACL {
  ...
  async get(options) {
    ...
    let links = API.parseLinkHeader(response.headers.link);

    if( links.acl ) {
      return links.acl.map(link => {
  ...
  async allACLAuthorizations(options) {
    ...
    options.headers.Accept = API.RDF_FORMATS.JSON_LD;
    let response = await API.get(options);
    if( response.response.statusCode !== 200 ) return {error: 'no access'};
    ...
    for( let i = 0; i < contains.length; i++ ) {
      ...
      if( agentClasses.length > 0 ) {
        ...
        for( var j = 0; j < agentClasses.length; j++ ) {
          ...
          let groupContainer = this._getContainer(JSON.parse(response.response.body));
  ...
  async authorizations(options) {
    ...
    let aclContainerPaths = await this.get(options);
    if( !aclContainerPaths ) return {};
  ...
  async add(options) {
    ...
    if( !aclPath ) {
      ...
      let aclPaths = await this.get(clone(options));
      if( !aclPaths ) throw new Error('Unable to find or access ACL for path: '+options.path);
      aclPath = aclPaths[0];
    }
  ...
  async _isPathAccessControlRoot(baseUrl, containerPath, aclPath) {
    ...
    if( response.response.statusCode === 200 ) {
      let container = this._getContainer(response.response.body);
