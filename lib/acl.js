...
class ACL {
  ...
  async add(options) {
    ...
    } else if( agentName.match(/^\//i) ) {
      agentName = agentName.replace(/\/$/, '').split('/').pop();
      let baseUrl = API.getBaseUrl(options)
      options.agent = `<${baseUrl}/${options.agent.replace(/^\//,'')}>`;
    } else {
      options.agent = `"${options.agent}"`;
    }
    ...
    options.headers.Slug = agentName;
    options.headers['Content-Type'] = API.RDF_FORMATS.TURTLE;
  ...
  addGroup(options) {
    options.agents = (options.agents || []).map(agent => {
      if( agent.match(/^http/i) ) {
        return `  foaf:member <${agent}> ;`;
      } else {
        return `  foaf:member "${agent}" ;`;
      }
    });
