...
class FinApi {
  ...
  async postEnsureSlug(options) {
    ...
    // check general error
    if( response.last.statusCode !== 201 ) {
      response.error = new Error('Failed to create container: '+response.last.statusCode);
    }
    ...
    if( createdSlug !== options.headers.Slug ) {
      ...
      let delResponse = await this.delete({
        path: location.replace(this.getBaseUrl(options), ''), 
        permanent: true
      });

      if( delResponse.last.statusCode !== 204 ) {
        response.setError('Failed to create container at location and failed to cleanup created container: '+options.headers.Slug);
        response.failedCleanupLocation = location;
      }

      response.appendResponse(delResponse);
      return response;
    }
  ...
  async startTransaction(options = {}) {
    ...
    let response = await _simpleRequest(req);
    if( response.error || !response.headers.location ) return response;
  ...
  async commitTransaction(options = {}) {
    ...
    let req = baseRequest('POST', options);
