...
class FinApi {
  ...
  async postEnsureSlug(options) {
    ...
    if( response.last.statusCode !== 201 ) {
      ...
      response.setError('Failed to create container: '+response.last.statusCode);
      return response;
    }
  ...
  isRdfContainer(response) {
    ...
    if( !response.headers.link ) return null;

    let link = this.parseLinkHeader(response.headers.link);
    if( !link.type ) return null;

    return (link.type.indexOf(this.LDP_TYPES.RDF_SOURCE) > -1);
  }
