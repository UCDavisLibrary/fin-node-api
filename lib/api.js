...
class FinApi {
  ...
  getBaseUrl(options) {
    return (options.host ? options.host : config.host) + this.createFcBasePath(options);
  }
  ...
  async delete(options) {
    ...
    try {
      if( options.permanent ) {
        let {response, body, authenticated} = await request(req);

        // if the initial delete fails, do not attempt to delete tombstone
        // 410 status code is 'gone'
        if( !this.isSuccess(response) && response.statusCode !== 410 ) {
