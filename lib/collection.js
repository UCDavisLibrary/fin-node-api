...
class Collection {
  ...
  async createRelationContainer(options) {
    ...
    let membershipResource = fcrPathUtils.joinUrlPath(API.getConfig().fcBasePath, options.path);
  ...
  /**
   * @method addItem
   * @description add a item to a collection.  Structure can be as follows:
   * 
   ...
   * @param {String} options.fsPath (optional) if provided, metadata, binary and itemMetadata will be assumed files
  ...
  async addItem(options) {
    ...
    if( options.id ) {
      ...
      itemId = (info.dir + '/' + info.name).replace(/^\//, '');
    }
    ...
    if( options.fsPath ) {
      if( options.data ) {
        bOptions.file = path.join(options.fsPath, options.data);
      } else if( fs.existsSync(path.join(options.fsPath, 'index.ttl')) ) {
        bOptions.file = path.join(options.fsPath, 'index.ttl');
      }
    } else {
    ...
    if( !itemMetadata && options.fsPath ) { // check for a .ttl file
      ...
      if( fs.existsSync(fs.join(options.fsPath, itemMetadata)) ) {
        itemMetadata = fs.readFileSync(fs.join(options.fsPath, itemMetadata), 'utf-8');
      }
    }
